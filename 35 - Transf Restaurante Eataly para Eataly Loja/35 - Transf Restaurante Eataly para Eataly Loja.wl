...
getALIMENTUM[codLoja_]:=Module[{sql,r,conn=marcheConn[],tab},
	sql=" 
		...
		WHERE 1=1
			   AND COD_PRODUTO IN (SELECT DISTINCT COD_PRODUTO FROM #TAB_AJUSTE_TEMP)
			   AND COD_LOJA in  (33)
...
rep=marcheTemplate[grid=Row@{"Pend\[EHat]ncias de emiss\[ATilde]o de NF da 33-Eataly Restaurante para 29-Eataly  em ",$now},createReport[s],1200, $logo]
...
getALIMENTUM[codLoja_]:=Module[{sql,r,conn=marcheConn[],tab},
	sql=" 
		...
		LEFT JOIN [192.168.0.13].BI.DBO.BI_CAD_PRODUTO AS CAD ON AJUSTE.COD_PRODUTO = CAD.COD_PRODUTO
		WHERE 1=1
				AND ISNULL(CAST((QTD_AJUSTE-QTD_EMITIDA) AS DOUBLE PRECISION),QTD_AJUSTE) < 0
				AND AJUSTE.COD_LOJA IN (`1`)
				--AND CUSTO IS NULL
...
$fileName1="Dash emissao de NF da 33-Eataly para 29-Eataly em "<>$now2<>".png";
Export["Dash emissao de NF da 33-Eataly para 29-Eataly em "<>$now2<>".png",rep];


$fileName="Pendencia de emiss\[ATilde]o 33-Eataly Restaurante para 29-Eataly Loja em "<>$now2<>".xlsx";
Export["Pendencia de emiss\[ATilde]o 33-Eataly Restaurante para 29-Eataly Loja em "<>$now2<>".xlsx",r["DataAll"]];		
...
marcheMail[
  		"[GE] Pend\[EHat]ncias de emiss\[ATilde]o de NF Eataly Restaurante PARA Eataly Loja em " <>$now 
